...
### New request-based job flow (availability requests)

- `POST /api/jobs/request`: User sends "Are you available?" request to a worker. Creates a pending job request (status: `pending`).
- `PUT /api/jobs/:jobId/accept`: Worker accepts the availability request — job `status` becomes `accepted`, `chatEnabled` is set to true and a chat is created.
- `PUT /api/jobs/:jobId/reject`: Worker rejects the availability request — job `status` becomes `rejected`.
- `PUT /api/jobs/:jobId/complete`: Worker marks the job complete — job `status` becomes `completed` and `chatEnabled` is set to false.

**Notes**:

- Job `status` now supports: `pending`, `accepted`, `in_progress`, `completed`, `cancelled`, `rejected`.
- Chats linked to a job are only usable while `job.chatEnabled === true`. Attempts to access or send messages when chat is disabled return `403` (Chat disabled).
- Reviews should be posted to `POST /api/jobs/:jobId/reviews` and are only allowed for completed jobs.
...
2.  **Install dependencies**:
    ...
    npm install
    ```
3.  **Set up environment variables**: Create a `.env` file in the root directory and add the necessary configuration, such as your database connection string and JWT secret.
